/*
    ...
    by Steve Gardner https://twitter.com/steveg3003/  
		...
		converted to pure Javascript (no jQuery) by Brigitte Jellinek
*/
let currentWeather;  // wichtigste globale  Variable!
...
let temperatureHolder = document.querySelector(".temp");
...
let innerRainHolder = [
  ...
  weatherContainer2.group(),
  weatherContainer3.group()
];
...
function init() {
  ...
  // 🖱 weather menu buttons -> when clicked, swith weather

  for (let w of weather) {
    let b = document.querySelector("#button-" + w.type);
    w.button = b;
    b.addEventListener("click", function(event) {
      // console.log(`switch to ${w.type}`);
      changeWeather(w); // closure!  w from the loop is kept in this function
    });
  }
...
function onResize() {
  ...
  sizes.card.width = card.clientWidth;
  sizes.card.height = card.clientHeight;
...
function makeRain() {
  ...
  let line = innerRainHolder[Math.floor(lineWidth)]
